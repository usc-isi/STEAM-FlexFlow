...
LegionRuntime::Logger::Category log_app("VGG16Sim");

void parse_input_args(char **argv, int argc, VGG16Config& config)
{
...
{
  FFConfig ffConfig;
  VGG16Config vgg16config;
  {
    const InputArgs &command_args = HighLevelRuntime::get_input_args();
    char **argv = command_args.argv;
    int argc = command_args.argc;
    parse_input_args(argv, argc, vgg16config);
    log_app.print("batchSize(%d) workersPerNodes(%d) numNodes(%d)",
        ffConfig.batchSize, ffConfig.workersPerNode, ffConfig.numNodes);
  }
  ffConfig.numNodes = vgg16config.nsimnode;
...
#if 0
  // Data Loader
  DataLoader data_loader(ff, &vgg16config, input, ff.label_tensor);
  ff.init_layers();
  //Start timer
  {
  ...
  for (int epoch = 0; epoch < ffConfig.epochs; epoch++) {
    ...
    for (int iter = 0; iter < iterations; iter++) {
      if (std::strlen(vgg16config.dataset_path) == 0) {
                       ...
                       Tensor input, Tensor label)
{
  ...
  TaskLauncher launcher(CUSTOM_CPU_TASK_ID_1,
      TaskArgument(alexnet, sizeof(VGG16Config)));
...
void DataLoader::load_entire_dataset(const Task *task,
  ...
  Context ctx, Runtime* runtime)
{
  const VGG16Config* alexnet = (VGG16Config*)task->args;
