...
int node_lookup_table(int node_id){
...
std::vector<uint64_t> rearrange_node_group(std::vector<uint64_t> ng) {

       std::vector<uint64_t> new_node_group;

       if(ng.size()==16) {

           new_node_group.push_back(0);
           new_node_group.push_back(4);
           new_node_group.push_back(8);
           new_node_group.push_back(12);
           new_node_group.push_back(1);
           new_node_group.push_back(5);
           new_node_group.push_back(9);
           new_node_group.push_back(13);
           new_node_group.push_back(2);
           new_node_group.push_back(6);
           new_node_group.push_back(10);
           new_node_group.push_back(14);
           new_node_group.push_back(3);
           new_node_group.push_back(7);
           new_node_group.push_back(11);
           new_node_group.push_back(15);
...
FFApplication::FFApplication(DynExpTopology* top, int cwnd, double pull_rate, int rlb_cutoff, 
        NdpRtxTimerScanner & nrts, NdpSinkLoggerSampling & sl, EventList & eventlist, FFAllReduceStrategy ars) 
    : topology(top), cwnd(cwnd), pull_rate(pull_rate), ndpRtxScanner(nrts), sinkLogger(sl), eventlist(eventlist), rlb_cutoff(rlb_cutoff),final_finish_time(0),n_finished_tasks(0), allreduce_strategy(ars) {
          FFApplication::total_apps++;
    fancy_ring = false;
    finished_once = false;
}
...
void FFApplication::load_taskgraph_flatbuf(std::string & taskgraph) {
    ...
    //
    //
    //uint64_t node_for_node_group = 0;
    //uint64_t andy_scaler = 6;
    //uint64_t node_scaled = 0;
    for (int i = 0; i < fbuf_tg->tasks()->size(); i++) {
		...
		//node_for_node_group = reinterpret_cast<uint64_t>(this_task.nexttasks()->Get(j)); 
		//node_scaled = node_for_node_group*andy_scaler;
		//node_group.push_back(node_scaled);

                node_group.push_back(reinterpret_cast<uint64_t>(this_task.nexttasks()->Get(j)));

               //  if (node_group.size()>4) {

                 //   node_group = rearrange_node_group(node_group);
          ...
          //  if (node_group.size()>4){
            ...
            //allreduce_strategy = FFApplication::FF_PS_AR;
...
FFTask::FFTask(FFApplication * ffapp, std::string type, FFDevice * device, uint64_t xfersize, 
    ...
    int src_node_from_device = 0;
    int dst_node_from_device = 0;

    int src_node_from_lut = 0;
    int dst_node_from_lut = 0;

    if (device->type == FFDevice::DEVICE_NW_COMM) {
         
         //src_node_from_device = device->from_node;
         //dst_node_from_device = device->to_node;

         //src_node_from_lut = node_lookup_table(src_node_from_device);
         //dst_node_from_lut = node_lookup_table(dst_node_from_device);

         //this->src_node = src_node_from_lut;
         //this->dst_node = dst_node_from_lut;
           

        this->src_node = device->from_node;
        this->dst_node = device->to_node;
//	this->src_node = this->src_node*6;
//	this->dst_node = this->dst_node*6;
    }
    else {
...
FFTask::FFTask(FFApplication * ffapp, FlatBufTaskGraph::SimTaskType tasktype, FFDevice * device,
    ...
    if (device->type == FFDevice::DEVICE_NW_COMM) {
        this->src_node = device->from_node;
        this->dst_node = device->to_node;
//	this->src_node = this->src_node;
//	this->dst_node = this->dst_node;
    }
    else {
        this->src_node = this->dst_node = -1; 
    }
...
void FFTask::cleanup() {
    ...
    //cout << "finished task " << this->
    cout << ffapp << " finished one task, nfin " << ffapp->n_finished_tasks << " ntot " << ffapp->tasks.size() << " type " << this->type << " now " << eventlist().now() << endl;
    if (ffapp->final_finish_time < finish_time) {
        ffapp->final_finish_time = finish_time;
    }
    //simtime_picosec delta = 5000;
...
void FFTask::start_flow() {
    ...
    cout << "[AR] " << src_node << " " << dst_node << " " << (uint64_t) this->xfersize << endl ; 
    cout << "[AR] " <<src_node << " " << dst_node << " " << xfersize << endl ; 
    //cout << ffapp << "non AR task: " << (uint64_t)this << " start flow (" << src_node << ", " << dst_node << endl;
    start_time = ready_time;
     

    //src_node = node_lookup_table(src_node);
    //dst_node = node_lookup_table(dst_node);
   //src_node = src_node*6;
   // dst_node = dst_node*6;
   // int src_node2 = src_node*6;
   // int dst_node2 = dst_node*6;

    int group1 = src_node/6;
    int group2 = dst_node/6;
    // if (group1 != group2){
     if (xfersize < ffapp->rlb_cutoff) { // priority flow, sent it over NDP
        std::cerr << "NDP regular task" << src_node << " to " << dst_node << "flow " << xfersize << std::endl;
	std::cerr << "Starting new NDP flow" << std::endl;
        // generate an NDP source/sink:
        NdpSrc* flowSrc = new NdpSrc(ffapp->topology, nullptr, nullptr, eventlist(), src_node, dst_node, taskfinish, this);
        ...
        NdpSink* flowSnk = new NdpSink(ffapp->topology, flowpacer, src_node, dst_node);
        ffapp->ndpRtxScanner.registerNdp(*flowSrc);

        // set up the connection event
        flowSrc->connect(*flowSnk, start_time);
        ...
        std::cerr << "RLB regular task" << src_node << " to " << dst_node << "flow" << xfersize << std::endl;
	std::cerr << "Starting new RLB flow" << std::endl;
        RlbSrc* flowSrc = new RlbSrc(ffapp->topology, NULL, NULL, eventlist(), src_node, dst_node);
        // debug:
        //cout << "setting flow size to " << vtemp[2] << " bytes..." << endl;
        flowSrc->set_flowsize(xfersize); // bytes

        RlbSink* flowSnk = new RlbSink(ffapp->topology, eventlist(), src_node, dst_node, taskfinish, this);
...
void FFRingAllreduce::doNextEvent() {
    ...
    if (node_group.size() == 1) {
        ...
        if (operator_size < 1500 * node_group.size() /* MTU */) {
...
void FFRingAllreduce::start_flow(int src_idx, int id) {
    ...
    cout << "[AR] " << src_node << " " << dst_node << " "  << operator_size/node_group.size() << endl ;
    //src_node = src_node*6;
    //dst_node = dst_node*6;

     //cout << ffapp <<  "FFapp " << ffapp << " AR task: " << (uint64_t)this << 
    ...
    // if (group1 != group2){
    if (operator_size/node_group.size() < ffapp->rlb_cutoff) { // priority flow, sent it over NDP
        std::cerr << "NDP ring all reduce" <<std::endl;
	std::cerr << "Starting new NDP flow" << std::endl;
        // generate an NDP source/sink:
        NdpSrc* flowSrc = new NdpSrc(ffapp->topology, nullptr, nullptr, eventlist(), src_node, dst_node, ar_finish_ring, f);
        flowSrc->setCwnd(ffapp->cwnd * Packet::data_packet_size()); // congestion window
        flowSrc->set_flowsize(operator_size/node_group.size()); // bytes
        ...
        // set up the connection event
        flowSrc->connect(*flowSnk, curr_round > 0 ? eventlist().now() : start_time + run_time);
        ...
        // generate an RLB source/sink:
        std::cerr << "RLB ring all reduce" << src_node << " to " << dst_node << std::endl;
	std::cerr << "Starting new RLB flow" << std::endl;
        RlbSrc* flowSrc = new RlbSrc(ffapp->topology, NULL, NULL, eventlist(), src_node, dst_node);
        // debug:
        //cout << "setting flow size to " << vtemp[2] << " bytes..." << endl;
        ...
        RlbSink* flowSnk = new RlbSink(ffapp->topology, eventlist(), src_node, dst_node, ar_finish_ring, f);
...
void ar_finish_ring(void * arinfo) {
    ...
    if (ar->finished_curr_round == (int)ar->node_group.size()) {
        ...
        if (ar->curr_round == 1 && (ar->operator_size / (2.0 * (ar->node_group.size() - 1) / ar->node_group.size())) <= 1500 * ar->node_group.size()) {
...
void ar_finish_newring(void * arinfo)
{
    ...
    if (ar->finished_curr_round[ring_idx] == (int)ar->node_group.size()) {
        ...
        if (ar->curr_round[ring_idx] == 1 && (ar->operator_size / (2.0 * (ar->node_group.size() - 1) / ar->node_group.size())) <= 1500 * ar->node_group.size()) {
            std::cerr << "early terminiate..." << std::endl;
            ar->finished_rings++;
        }
...
void FFPSAllreduce::start_flow(int node_idx, int direction) {
   ...
   // dst_node = node_lookup_table(dst_node);
    ...
    //if (group1 != group2){
     ...
     if (operator_size < ffapp->rlb_cutoff) { // priority flow, sent it over NDP
        std::cerr << "NDP PS AllReduce " << src_node << " to "<<dst_node << " flow " << operator_size << std::endl;
	std::cerr << "Starting new NDP flow" << std::endl;
        // generate an NDP source/sink:
        NdpSrc* flowSrc = new NdpSrc(ffapp->topology, nullptr, nullptr, eventlist(), src_node, dst_node, ar_finish_ps, f);
        std::cerr << "generated flow src" << std::endl;
         flowSrc->setCwnd(ffapp->cwnd * Packet::data_packet_size()); // congestion window
        std::cerr << "set congestion window" << std::endl;
        flowSrc->set_flowsize(operator_size); // bytes
        std::cerr << "set flow size" << std::endl;
        // Set the pull rate to something reasonable.
        // we can be more aggressive if the load is lower
        NdpPullPacer* flowpacer = new NdpPullPacer(eventlist(), ffapp->pull_rate); // 1 = pull at line rate
        //NdpPullPacer* flowpacer = new NdpPullPacer(eventlist(), .17);
        std::cerr << "set pacer" << std::endl;
        NdpSink* flowSnk = new NdpSink(ffapp->topology, flowpacer, src_node, dst_node);
        std::cerr << "set sink" << std::endl;
        ffapp->ndpRtxScanner.registerNdp(*flowSrc);
        std::cerr << "set scanner" << std::endl;
        // set up the connection event
        flowSrc->connect(*flowSnk, curr_round > 0 ? eventlist().now() : start_time);
        std::cerr << "set connection" << std::endl;
        ffapp->sinkLogger.monitorSink(flowSnk);
        std::cerr << "set logger" << std::endl;
    }  else { // background flow, send it over RLB

        // generate an RLB source/sink:
        std::cerr << "RLB PS AllReduce " << src_node << " to " << dst_node << " flow " << operator_size << std::endl;
	std::cerr << "Starting new RLB flow" << std::endl;
        RlbSrc* flowSrc = new RlbSrc(ffapp->topology, NULL, NULL, eventlist(), src_node, dst_node);
        // debug:
        //cout << "setting flow size to " << vtemp[2] << " bytes..." << endl;
        ...
        RlbSink* flowSnk = new RlbSink(ffapp->topology, eventlist(), src_node, dst_node, ar_finish_ps, f);
...
if (operator_size/node_group.size() < ffapp->rlb_cutoff) { // priority flow, sent it over NDP
	...
	std::cerr << "Starting new NDP flow" << std::endl;
        // generate an NDP source/sink:
        NdpSrc* flowSrc = new NdpSrc(ffapp->topology, nullptr, nullptr, eventlist(), src_node, dst_node, ar_finish_dps, this);
        flowSrc->setCwnd(ffapp->cwnd * Packet::data_packet_size()); // congestion window
        flowSrc->set_flowsize(operator_size/node_group.size()); // bytes
        ...
        // generate an RLB source/sink:
        std::cerr << "RLB " << src_node << " to " << dst_node << std::endl;
	std::cerr << "Starting new RLB flow" << std::endl;
        RlbSrc* flowSrc = new RlbSrc(ffapp->topology, NULL, NULL, eventlist(), src_node, dst_node);
        // debug:
        //cout << "setting flow size to " << vtemp[2] << " bytes..." << endl;
        ...
        RlbSink* flowSnk = new RlbSink(ffapp->topology, eventlist(), src_node, dst_node, ar_finish_dps, this);
