                                   ...
                                   TaskOptions& output)
{
  ...
  if (task.task_id == CUSTOM_SIMULATION_TASK_ID) {
    output.initial_proc = all_gpus[0];
    return;
  }
  if (task.task_id == CUSTOM_SIMULATION_TASK_ID_2) {
    output.initial_proc = all_gpus[0];
    return;
  }
  ...
  if (task.task_id == CUSTOM_TEST_TASK_ID_1) {
    output.initial_proc = all_gpus[0];
    return;
  }
  if (task.task_id == CUSTOM_TEST_TASK_ID_2) {
    output.initial_proc = all_gpus[0];
    return;
  }
  if (task.task_id == CUSTOM_TEST_TASK_ID_3) {
    output.initial_proc = all_gpus[0];
    return;
  }
  ...
  if (task.task_id == CUSTOM_MEASUREMENT_TASK_ID_2) {
    output.initial_proc = all_gpus[0];
    return;
  }
  if (task.task_id == CUSTOM_MEASUREMENT_TASK_ID_3) {
    output.initial_proc = all_gpus[0];
    return;
  }
