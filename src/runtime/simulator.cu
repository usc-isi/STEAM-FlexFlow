...
: LogicalTaskgraphBasedSimulator(model, handler, memory, machine)
{
  ...
  this->topofinder = 
    new NSDI22Heuristic(this->machine, model->config.node_degree, 4);
}

void DLSSchedulerBasedSimulator::simulation_task(const Task *task,
  const std::vector<PhysicalRegion> &regions, Context ctx, Runtime *runtime) 
{
  ...
  Simulator* simulator = new DLSSchedulerBasedSimulator(model, model->handlers[0], gpu_mem, machine);
  if (model->config.mfile != "") {
    model->load_measurement(simulator, model->config.mfile);
  }
  // DemandHeuristicNetworkOptimizer *dhopt = 
