...
class TwoDimTorusW2TURNRouting : public NetworkRoutingStrategy {
public: 
  TwoDimTorusW2TURNRouting(const ConnectionMatrix & c, 
                           const std::map<size_t, CommDevice*>& devmap, 
                           int nrows,
                           int ncols,
                           int nextras,
  ...
  inline int even_choose_dir(int s, int d, int m, int k) const;
  inline bool on_the_way(int s, int d, int m, int k) const;
  inline bool route_in_mindir_end(int p1, int p2, int p3, int k) const;
  inline bool route_in_mindir_mid(int p1, int p2, int p3, int p4, int p5, int k) const;
  void route_x(int srcx, int dstx, int y, int dir, int k, Route& route);
...
private:
  ...
  int nextras;
  int total_devs;
  std::unordered_map<uint64_t, EcmpRoutes> cached_routes;
};
...
/**
 * geenrate a 2D topology (degree 4).
 * The topology is generated by creating the closest k-ary 2d torus that is
 * larger and then add row-column to reach the number of nodes. 
 * 
 * The Final torus will look like 
 * ----- n rows ------
 * |                 |
 * |                 |
 * n cols         n cols
 ...
 * - n extras -
 */
class TwoDimTorusNetworkTopologyGenerator {
public: 
